...
# 单阶段算法 MB-WFD
def mb_wfd(items, bin_capacity, initial_bins):
    bins = []
    while len(bins) < initial_bins:  # 确保初始有足够的箱子打开
        bins.append([])

    items_sorted = sorted(items, reverse=True)  # 下降排序以优化装箱
...
# 运行实验
def run_experiment(file_path, bin_capacity=1000, initial_bins=10):
    ...
    for items in datasets:
        ...
        bins = mb_wfd(items, bin_capacity, initial_bins)
