...
# 第一阶段装箱算法 IC-FFD
def first_stage_algorithm(items, bin_capacity, threshold):
    bins = []
    ...
    for index in range(len(items_sorted)):
        ...
        if (index + 1) % checkpoints == 0:
            ...
            print("空间平均利用率",np.mean([sum(b) / bin_capacity for b in bins]))
            print("时间平均利用率",(end_time - start_time)/(index+1))
            print(f"Performance at {index + 1} items: {performance_score}")
            print("bins中数量",len(bins))
            print("阈值",threshold)
            print(bins)
            print()
            threshold = adjust_threshold(performance_score, threshold)
            checkpoint_counter += 1
            if len(bins) > threshold:  # 边装箱边检查是否超过阈值
                return bins, True, remaining_items  # 返回当前箱子列表和一个标志表示超过阈值以及剩余物品
...
def second_stage_algorithm(remaining_items, bins, bin_capacity):
    items_sorted = sorted(remaining_items, reverse=True)  # 如果需要，可以调整排序方式
...
print(f"Total time spent: {average_time_spent} seconds")
print("--------------------------------------------------------------------------------")
print(f"Total datasets processed: {average_deviation_percents}%")